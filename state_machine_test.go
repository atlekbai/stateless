...
func TestInitialTransition_WithReentry(t *testing.T) {
	...
	sm.Configure(StateA).
		...
		OnEntry(func() { order++; onEntryStateAfired = order }).
		OnExit(func() { order++; onExitStateAfired = order }).
		PermitReentry(TriggerX)

	sm.Configure(StateB).
		SubstateOf(StateA).
		OnEntry(func() { order++; onEntryStateBfired = order }).
		OnExit(func() { order++; onExitStateBfired = order })
...
func TestInitialTransition_VerifyNotEnterSuperstateWhenDoingInitialTransition(t *testing.T) {
	...
	sm.Configure(StateB).
		InitialTransition(StateC).
		OnEntry(func() { sm.Fire(TriggerY, nil) }).
...
func TestInitialTransition_SubStateOfSubstateOnEntryCountAndOrder(t *testing.T) {
	...
	sm.Configure(StateA).
		OnEntry(func() { onEntryCount += "A" }).
		Permit(TriggerX, StateB)

	sm.Configure(StateB).
		OnEntry(func() { onEntryCount += "B" }).
		InitialTransition(StateC)

	sm.Configure(StateC).
		OnEntry(func() { onEntryCount += "C" }).
		InitialTransition(StateD).
		SubstateOf(StateB)

	sm.Configure(StateD).
		OnEntry(func() { onEntryCount += "D" }).
		SubstateOf(StateC)
...
func TestInitialTransition_TransitionEventsOrdering(t *testing.T) {
	...
	sm.Configure(StateA).
		Permit(TriggerX, StateB).
		OnExit(func() { actualOrdering = append(actualOrdering, "OnExitA") })

	sm.Configure(StateB).
		InitialTransition(StateC).
		OnEntry(func() { actualOrdering = append(actualOrdering, "OnEntryB") })

	sm.Configure(StateC).
		SubstateOf(StateB).
		OnEntry(func() { actualOrdering = append(actualOrdering, "OnEntryC") })
